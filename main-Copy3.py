...
# 1. データローダーの作成
# Define your data augmentation and normalization transforms
...
class VQADataset(torch.utils.data.Dataset):
    def __init__(self, df_path, image_dir, transform=None, answer=True, class_mapping_path=None):
...
#         self.idx2question = {v: k for k, v in self.question2idx.items()}  # 逆変換用の辞書(question)
        ...
        if self.answer:
            ...
            if class_mapping_path:
                ...
                for idx, row in class_mapping.iterrows():
                    answer = row['answer']
...
def main():
    ...
    train_dataset = VQADataset(df_path="./data/train.json", image_dir="./data/train", transform=transform, class_mapping_path=class_mapping_path)
    test_dataset = VQADataset(df_path="./data/valid.json", image_dir="./data/valid", transform=transform, answer=False, class_mapping_path=class_mapping_path)
    test_dataset.update_dict(train_dataset)

    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=128, shuffle=True)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=False)

    n_answer = train_dataset.get_answer_vocab_size()
